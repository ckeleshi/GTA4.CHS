 ...
 *      !!!! NOTICE !!!! --> Any derived which implements/reimplements restore()
 * should implement a destructor calling it
 */
class scoped_base {
  public:
    virtual ~scoped_base() {}
...
class scoped_basic : public scoped_base {
  ...
  public:
    ...
    // Problems may arise if someone else hooked the same place using the same
    // method
...
class scoped_write : public scoped_basic<bufsize_> {
  public:
    // Save buffer at @addr with @size and virtual protect @vp and then
    ...
    // Save buffer at @addr with size sizeof(@value) and virtual protect @vp and
    // then overwrite it with @value
    template <class T>
    ...
    scoped_write &operator=(scoped_write &&rhs) {
        scoped_basic<bufsize_>::operator=(std::move(rhs));
        return *this;
    }
};
...
class scoped_fill : public scoped_basic<bufsize_> {
  public:
    // Fills memory at @addr with value @value and size @size and virtual
    // protect @vp
 ...
 */
template <size_t bufsize_> class scoped_nop : public scoped_basic<bufsize_> {
  public:
    // Makes NOP at @addr with value @value and size @size and virtual protect
    // @vp
...
class scoped_jmp : public scoped_basic<5> {
  public:
    // Makes NOP at @addr with value @value and size @size and virtual protect
    // @vp
    memory_pointer_raw make_jmp(memory_pointer_tr at, memory_pointer_raw dest,
                                bool vp = true) {
        this->save(at, 5, vp);
    ...
    scoped_jmp &operator=(scoped_jmp &&rhs) {
        scoped_basic<5>::operator=(std::move(rhs));
        return *this;
    }
...
class scoped_call : public scoped_basic<5> {
  public:
    // Makes NOP at @addr with value @value and size @size and virtual protect
    // @vp
    memory_pointer_raw make_call(memory_pointer_tr at, memory_pointer_raw dest,
                                 bool vp = true) {
        this->save(at, 5, vp);
    ...
    _MSC_VER >= 1800 // C++11 or MSVC 2013 required for variadic templates

/*
 *  function_hooker_manager
 *      Manages many function_hookers that points to the same address
 *      The need for this function arises because otherwise we would only be
 * able to allow one hook per address using function_hookers This manager takes
 * care of the amount of hooks placed in a particular address, calls the hooks
 * and unhooks when necessary.
 */
...
class function_hooker_manager : protected scoped_call {
  private:
    ...
    // Adds a new item to the association map (or override if already in the
    // map)
  ...
  public:
    ...
    static Ret call_hooks(Args &...args) {
        ...
        if (manager.assoc.size() ==
            0) // This may be uncommon but may happen (?), no hook installed
        ...
        if (manager.assoc.size() == 1) {
            // We have only one hook, just use it directly no need to go further
            // in complexity
            auto &functor = manager.assoc.begin()->second;
            return functor(std::move(original), args...);
        } else {
            // Build a serie of functors which captures the previous functor
            // sending it to the next functor, that's what would happen if the
            // hooks took place independent of the template staticness (AAAAAAA)
            func_type next = std::move(original);
            for (auto it = manager.assoc.begin(); it != manager.assoc.end();
                 ++it) {
  ...
  public:
    // Installs a hook associated with the function_hooker 'hooker' which would
    // call the specified 'functor' We need an auxiliar function pointer 'ptr'
    // (to abstract calling conventions) which should forward itself to
    // ^call_hooks
    void make_call(const ToManage &hooker, functor_type functor,
                   memory_pointer_raw ptr) {
        ...
        if (!this->has_hooked) {
            ...
            this->original =
                (func_type_raw)(void *)scoped_call::make_call(hooker.addr, ptr)
                    .get();
    ...
    // If the number of hooks reaches zero after the remotion, a restore will
    // take place
    ...
    // This work as a shared pointer to avoid the static destruction of the
    // manager even when a static function_hooker still wants to use it.
    static std::shared_ptr<function_hooker_manager> instance() {
        static auto fm_ptr = std::shared_ptr<function_hooker_manager>(
            new function_hooker_manager());
        return fm_ptr;
    }
};
 ...
 *      Base for any function_hooker, this class manages the relationship with
 * the function hooker manager
 */
...
class function_hooker_base : public scoped_base {
  public:
    ...
    using manager_type =
        function_hooker_manager<function_hooker_base, Ret, Args...>;
  ...
  public:
    ...
    //
    virtual ~function_hooker_base() { this->restore(); }
    ...
    function_hooker_base(function_hooker_base &&rhs)
        : scoped_base(std::move(rhs)), has_call(rhs.has_call),
          manager(rhs.manager) // (don't move the manager!, every
                               // function_hooker should own one)
    {
    ...
    function_hooker_base &operator=(function_hooker_base &&rhs) {
        ...
        this->manager = rhs.manager; // (don't move the manager! every
                                     // function_hooker should own one)
        return *this;
    }

    // Deriveds should implement a proper make_call (yeah it's virtual so
    // derived-deriveds can do some fest)
    ...
    bool has_hooked() { return this->has_call; }

  private:
    bool has_call = false; // Has a hook installed?
    std::shared_ptr<manager_type>
        manager; // **EVERY** function_hooker should have a ownership over it's
                 // manager_type this prevents the static destruction of the
                 // manager_type while it may be still needed.
  ...
  protected: // Forwarders to the function hooker manager
    ...
    static Ret call_hooks(Args &...a) { return manager_type::call_hooks(a...); }
};
 ...
 */
template <uintptr_t addr1, class Prototype> class function_hooker;
...
class function_hooker<addr1, Ret(Args...)>
  ...
  private:
    ...
    // The hook caller
    static Ret call(Args... a) { return base::call_hooks(a...); }
 ...
 */
template <uintptr_t addr1, class Prototype> struct function_hooker_stdcall;

template <uintptr_t addr1, class Ret, class... Args>
struct function_hooker_stdcall<addr1, Ret(Args...)>
    : public function_hooker_base<addr1, Ret(__stdcall *)(Args...), Ret,
                                  Args...> {
  private:
    using base =
        function_hooker_base<addr1, Ret(__stdcall *)(Args...), Ret, Args...>;

    // The hook caller
    static Ret __stdcall call(Args... a) { return base::call_hooks(a...); }

  public:
    // Constructors, move constructors, assigment operators........
    function_hooker_stdcall() = default;
    function_hooker_stdcall(const function_hooker_stdcall &) = delete;
    function_hooker_stdcall(function_hooker_stdcall &&rhs)
        : base(std::move(rhs)) {}
    function_hooker_stdcall &
    operator=(const function_hooker_stdcall &rhs) = delete;
 ...
 */
template <uintptr_t addr1, class Prototype> struct function_hooker_fastcall;

template <uintptr_t addr1, class Ret, class... Args>
struct function_hooker_fastcall<addr1, Ret(Args...)>
    : public function_hooker_base<addr1, Ret(__fastcall *)(Args...), Ret,
                                  Args...> {
  private:
    using base =
        function_hooker_base<addr1, Ret(__fastcall *)(Args...), Ret, Args...>;

    // The hook caller
    static Ret __fastcall call(Args... a) { return base::call_hooks(a...); }

  public:
    // Constructors, move constructors, assigment operators........
    function_hooker_fastcall() = default;
    function_hooker_fastcall(const function_hooker_fastcall &) = delete;
    function_hooker_fastcall(function_hooker_fastcall &&rhs)
        : base(std::move(rhs)) {}
    function_hooker_fastcall &
    operator=(const function_hooker_fastcall &rhs) = delete;
 ...
 */
template <uintptr_t addr1, class Prototype> struct function_hooker_thiscall;

template <uintptr_t addr1, class Ret, class... Args>
struct function_hooker_thiscall<addr1, Ret(Args...)>
    : public function_hooker_base<addr1, Ret(__thiscall *)(Args...), Ret,
                                  Args...> {
  private:
    using base =
        function_hooker_base<addr1, Ret(__thiscall *)(Args...), Ret, Args...>;

    // The hook caller
    static Ret __thiscall call(Args... a) { return base::call_hooks(a...); }

  public:
    // Constructors, move constructors, assigment operators........
    function_hooker_thiscall() = default;
    function_hooker_thiscall(const function_hooker_thiscall &) = delete;
    function_hooker_thiscall(function_hooker_thiscall &&rhs)
        : base(std::move(rhs)) {}
    function_hooker_thiscall &
    operator=(const function_hooker_thiscall &rhs) = delete;
 ...
 */
template <class T> inline T &add_static_hook(T &&hooker) {
    static std::list<T> a;
 ...
 */
template <class T, class F> inline T make_function_hook(F functor) {
    T a;
 ...
 *  'T' must be any function_hooker object
 */
template <class T, class F> inline T &make_static_hook(F functor) {
    return add_static_hook(make_function_hook<T>(std::move(functor)));
}

// TODO when we have access to C++14 add a make_function_hook,
// make_stdcall_function_hook, and so on the problem behind implement it with
// C++11 is that lambdas cannot be generic and the first param of a hook is a
// functor pointing to the previous call pointer
