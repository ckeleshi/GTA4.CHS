 ...
 */
template <size_t bufsize_ = 10> class scoped_write : public scoped_basic<bufsize_>
{
  public:
    ...
    template <class T> void write(memory_pointer_tr addr, T value, bool vp = false)
    {
 ...
 */
template <size_t bufsize_ = 5> class scoped_fill : public scoped_basic<bufsize_>
{
  public:
 ...
 */
template <class ToManage, class Ret, class... Args> class function_hooker_manager : protected scoped_call
{
  private:
  ...
  public:
    ...
    static Ret call_hooks(Args &...args)
    {
        ...
        func_type original = [&manager](Args... args) -> Ret { return manager.original(args...); };
        ...
        else
        {
            ...
            for (auto it = manager.assoc.begin(); it != manager.assoc.end(); ++it)
            {
                ...
                next = [functor, next](Args... args) -> Ret { return functor(next, args...); };
            }
            return next(args...);
        }
    }
 ...
 */
template <uintptr_t addr1, class FuncType, class Ret, class... Args> class function_hooker_base : public scoped_base
{
  public:
  ...
  public:
    ...
    function_hooker_base(function_hooker_base &&rhs)
        : scoped_base(std::move(rhs)), has_call(rhs.has_call), manager(rhs.manager) // (don't move the manager!, every
  ...
  private:
    ...
    std::shared_ptr<manager_type> manager; // **EVERY** function_hooker should have a ownership over it's
...
class function_hooker<addr1, Ret(Args...)> : public function_hooker_base<addr1, Ret (*)(Args...), Ret, Args...>
{
  private:
