...
/*
    ...
    INJECTOR_GVM_HAS_TRANSLATOR
        If defined, the user should provide their own
   address_manager::translator function. That function is responssible for
   translating a void pointer (that mayn't be an actual pointer) into an actual
   address. The meaning of that void pointer will be made by YOU when you send
   it to the functions that receive pointers on this library. The default
   translator does nothing but returns that void pointer as the address.

    INJECTOR_GVM_OWN_DETECT
        If defined,  the user should provide it's own game detection function
   thought game_version_manager::Detect By default it provide an good detection
   for the Grand Theft Auto series.

    INJECTOR_GVM_PLUGIN_NAME
        If this is defined, it will be used as the plugin name used at error
   messages. By default it will use ""Unknown Plugin Name"
    ...
    INJECTOR_OWN_GVM
        If defined, the game_version_manager should be implemented by the user
   before including this library. By default it provides a nice gvm for Grand
   Theft Auto series
*/
#include "gvm/gvm.hpp"
 ...
 *      A memory pointer class that is capable of many operations, including
 * address translation MemTranslator is the translator functor
 */
...
/*
 *  memory_pointer_tr
 *      Stores a basic_memory_pointer<Tr> as a raw pointer from translated
 * pointer
 */
...
/*
 *  AdjustPointer
 *      Searches in the range [@addr, @addr + @max_search] for a pointer in the
...
/*
 *  MakeJA
 *      Creates a JA instruction at address @at that jumps if above into address
