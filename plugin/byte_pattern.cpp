...
byte_pattern::result_type byte_pattern::get() const { return _results; }

memory_pointer byte_pattern::get_first() const { return this->get(0); }
...
byte_pattern &byte_pattern::set_pattern(const void *pattern_binary,
                                        size_t size) {
    this->_bpattern.clear();
    this->_mask.clear();
    this->_bpattern.assign(reinterpret_cast<const uint8_t *>(pattern_binary),
                           reinterpret_cast<const uint8_t *>(pattern_binary) +
                               size);
...
byte_pattern &byte_pattern::find_pattern(const void *pattern_binary,
                                         size_t size) {
...
pair<uint8_t, uint8_t> byte_pattern::parse_sub_pattern(const std::string &sub) {
    ...
    if (sub.size() == 1) {
        ...
        if (sub[0] == '?' && sub[1] == '?') {
            ...
            result.first =
                ((digit_to_value(sub[0]) << 4) | digit_to_value(sub[1]));
            result.second = 0xFF;
        }
    } else {
        throw invalid_argument("Could not parse pattern.");
...
void byte_pattern::get_module_range(memory_pointer module) {
    ...
    for (int i = 0; i < ntHeader->FileHeader.NumberOfSections; i++) {
        ...
        auto secSize = sec->SizeOfRawData != 0 ? sec->SizeOfRawData
...
size_t byte_pattern::count() const { return this->_results.size(); }

bool byte_pattern::has_size(size_t expected) const {
    return (this->_results.size() == expected);
}

bool byte_pattern::empty() const { return this->_results.empty(); }
...
void byte_pattern::bm_search() {
    ...
    uint8_t *range_end =
        reinterpret_cast<uint8_t *>(this->_range.second - pattern_len);

    ptrdiff_t index;

    while (range_begin <= range_end) {
        for (index = pattern_len - 1; index >= 0; --index) {
            if ((pbytes[index] & pmask[index]) !=
                (range_begin[index] & pmask[index])) {
                break;
            }
        }
...
std::string byte_pattern::make_bytes_literal(memory_pointer pointer,
                                             std::size_t length) {
    std::string result;
