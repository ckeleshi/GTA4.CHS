...
void *__fastcall CFont::LoadTextureCB(void *pDictionary, int, uint hash) {
    void *result =
        plugin.game.Dictionary_grcTexturePC_GetElementByKey(pDictionary, hash);
...
float CFont::GetStringWidthRemake(const GTAChar *str, bool get_all) {
    ...
    while (true) {
        ...
        if (chr == 0) {
            ...
            int token_type =
                plugin.game.Font_ParseToken(++str, token_string, &token_data);
            ...
            if (token_data.f110 == 0) {
                ...
                if (token_type < 256 || token_type > 300) {
                    ...
                    if (gta_string::gtaWcslen(token_string) > 0) {
                        // 91C122
                        plugin.game.Font_AddTokenStringWidth(
                            token_string, &current_width, render_index);
                        had_word = true;
                    }
                } else {
                    // 91C0C3
                    current_width += plugin.game.game_addr
                                         .pFont_ButtonWidths[token_type - 255] *
                ...
                for (int token_index = 0; token_index < 4; ++token_index) {
                    ...
                    case 1: {
                        // 91BF26
                        current_width +=
                            plugin.game.game_addr
                                .pFont_ButtonWidths[token_data.f0[token_index] -
                                                    255] *
                            using_details.fBlipScaleX;
                        had_word = true;
                    ...
                    case 2: {
                        // 91BF53
                        plugin.game.Font_AddTokenStringWidth(
                            token_data.f10[token_index], &current_width,
                            render_index);
                        had_word = true;
                        break;
                    }
...
float CFont::GetCHSCharacterSizeNormal() {
    auto render_index = plugin.game.Font_GetRenderIndex();
    auto &using_details = plugin.game.game_addr.pFont_Details[render_index];
    auto &using_font_data =
        plugin.game.game_addr.pFont_Datas[using_details.nFont];

    float extra_width =
        using_font_data
            .fWidthOfSpaceBetweenChars[using_details.nExtraWidthIndex];

    return (((fChsWidth + fChsWidthFix + extra_width) /
                 *plugin.game.game_addr.pFont_ResolutionX +
...
float CFont::GetCHSCharacterSizeDrawing(bool use_extra_width) {
    ...
    auto &using_font_data =
        plugin.game.game_addr.pFont_Datas[render_state->nFont];

    if (use_extra_width) {
        extra_width =
            using_font_data
                .fWidthOfSpaceBetweenChars[render_state->nExtraWidthIndex];
    }
...
float CFont::GetCharacterSizeDrawingDispatch(GTAChar chr,
                                             bool use_extra_width) {
    if (IsNativeChar(chr + 0x20)) {
...
void CFont::PrintCHSChar(float x, float y, GTAChar chr) {
    ...
    float screen_character_width =
        (fChsWidth / *plugin.game.game_addr.pFont_ResolutionX +
         render_state->fEdgeSize) *
        render_state->fScaleX;
    ...
    texture_rect.top_right.y =
        (row - 0.045f) * fSpriteHeight / fTextureResolution +
        8.0f / fTextureResolution;
    if (texture_rect.top_right.y > 1.0f) {
        texture_rect.top_right.y = 1.0f;
    }
    texture_rect.bottom_left.y =
        (row - 0.045f) * fSpriteHeight / fTextureResolution +
        79.0f / fTextureResolution - 0.001f / fRatio + 0.0048f / fRatio;
    texture_rect.bottom_left.x = column / fTextureColumnsCount;
    texture_rect.top_right.x =
        column / fTextureColumnsCount + texture_sprite_width;
    ...
    plugin.game.Font_Render2DPrimitive(&screen_rect, &texture_rect,
                                       render_state->field_18, false);
}

void CFont::PrintCharDispatch(float x, float y, GTAChar chr, bool buffered) {
    if (plugin.game.game_addr.pFont_RenderState->TokenType != 0 ||
        IsNativeChar(chr + 0x20)) {
        ...
        if (-(GetCHSCharacterSizeDrawing(true) /
              plugin.game.game_addr.pFont_RenderState->fScaleX) > x ||
            x > 1.0f) {
            return;
        }
