...
void register_patches()
{
    ...
    // 邮件回复
    // 要用的函数的地址
    plugin.game.game_addr.fnMailAppendByteString = injector::aslr_ptr(0x451450).get();
    ...
    // 读utf8序列
    // 用到"!--"的函数
    injector::MakeInline<mr_read_eax_edx>(injector::aslr_ptr(0x5B5DC4).get(), injector::aslr_ptr(0x5B5DC4 + 7).get());

    // IDA Names: const CHtmlParser::`vftable'
    // 虚表的第三个函数
    injector::MakeInline<mr_read_edx_eax>(injector::aslr_ptr(0x450741).get(), injector::aslr_ptr(0x450741 + 7).get());

    // CHtmlParser虚表第三个函数开头调用了MailGetStringChar
    // 在MailGetStringChar里面
    // 读utf8序列
    injector::MakeInline<mr_read_eax_edx>(injector::aslr_ptr(0x45064C).get(), injector::aslr_ptr(0x45064C + 7).get());

    // 在上述用到"!--"的函数里面
    // 读上一个utf8序列
    injector::MakeInline<mr_read_back_esi_eax>(injector::aslr_ptr(0x5B5DA3).get(),
                                               injector::aslr_ptr(0x5B5DA3 + 17).get());

    injector::MakeInline<mr_read_back_esi_eax>(injector::aslr_ptr(0x5B5DFA).get(),
                                               injector::aslr_ptr(0x5B5DFA + 17).get());

    // 在上述CHtmlParser虚表第三个函数里面
    // 读上一个utf8序列
    injector::MakeInline<mr_read_back_eax_ecx>(injector::aslr_ptr(0x45062A).get(),
                                               injector::aslr_ptr(0x45062A + 18).get());

    // 在上述用到"!--"的函数里面
    // 写utf8序列
    injector::MakeInline<mr_write_esi_ecx>(injector::aslr_ptr(0x5B5F92).get(), injector::aslr_ptr(0x5B5F92 + 13).get());

    // 在上述CHtmlParser虚表第三个函数里面
    // 写utf8序列
    injector::MakeInline<mr_write_esi_eax>(injector::aslr_ptr(0x4509AF).get(), injector::aslr_ptr(0x4509AF + 19).get());

    // 在上述CHtmlParser虚表第三个函数调用的函数里面
    // 写utf8序列
    injector::MakeInline<mr_write_edi_ebp>(injector::aslr_ptr(0x450ECA).get(), injector::aslr_ptr(0x450ECA + 8).get());

    // Native: CONVERT_THEN_ADD_STRING_TO_HTML_SCRIPT_OBJECT(72EC0AA6)的转换过程里面
    plugin.game.game_addr.pDLCTruncateBuffer = injector::aslr_ptr(0x10F4F58).get();

    // DLC不支持日语而加的转换逻辑
    injector::MakeInline<dlc_truncate>(injector::aslr_ptr(0x7B4B81).get(), injector::aslr_ptr(0x7B4B81 + 8).get());
