...
void CHtmlTextExport::ExportHtml(const path_type &input_folder) {
    ...
    for (; it != std::filesystem::recursive_directory_iterator{}; ++it) {
        ...
        if (filename.extension() == ".whm") {
            ExportText(filename.replace_extension(".txt"),
                       ExtractWhmStrings(it->path(), hashes));
        }
    }
}

void CHtmlTextExport::GenerateDataBase(const path_type &input_folder,
...
void CHtmlTextExport::ExportText(
    ...
    for (auto &entry : container) {
        std::fprintf(out, ";0x%08X=%s\n", entry.hash,
                     Windows1252ToUtf8(entry.str).c_str());
...
std::vector<CHtmlTextExport::exported_text_entry>
CHtmlTextExport::LoadText(const std::filesystem::path &filename) {
    ...
    while (std::getline(stream, line)) {
        ...
        if (std::regex_match(line, matches, entry_regex)) {
            ...
            //忽略空白文本&原译一致的文本
            if (!IsBlankText(entry.str) &&
                entry.hash != fnv_hash::hash_string(entry.str, false)) {
...
std::vector<uchar> CHtmlTextExport::UnpackWhm(const path_type &input_file) {
    ...
    if (file) {
        ...
        uncompress(uncompressed_bytes.data(), &uncompressed_size,
                   compressed_bytes.data(), compressed_size);
    }

    return uncompressed_bytes;
}

void CHtmlTextExport::ExtractNodeStrings(
    CHtmlNode *node, std::vector<uchar> &block,
...
void CHtmlTextExport::TryAppendString(
    std::vector<exported_text_entry> &container, char *ptr,
    std::set<std::size_t> &hashes) {
    //合法的文本类型:
    // 1) 包含EFIGS字母
    // 2) 非网址
    ...
    //判断字符是不是游戏中url会出现的字符
    auto validate_url_char = [validate_digit_char,
                              validate_english_char](char chr) {
        auto ucr = *reinterpret_cast<uchar *>(&chr);

        return validate_english_char(chr) || validate_digit_char(chr) ||
               ucr == '.' || ucr == '%' || ucr == '@' || ucr == '-' ||
               ucr == '_';
    };
    ...
    auto validate_string = [validate_efigs_char,
                            validate_url](std::string_view view) {
        //包含EFIGS字母且不是网址
...
std::vector<CHtmlTextExport::exported_text_entry>
CHtmlTextExport::ExtractWhmStrings(const std::filesystem::path &filename,
                                   std::set<std::size_t> &hashes) {
    std::vector<exported_text_entry> container;
    ...
    if (!block.empty()) {
        ...
        ExtractNodeStrings(p_doc->m_pBody.locate(block), block, container,
                           hashes);
    }
...
std::string CHtmlTextExport::Windows1252ToUtf8(const std::string &str) {
    ...
    for (auto chr : str) {
        utf8::append(*reinterpret_cast<uchar *>(&chr),
